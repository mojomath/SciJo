...
fn minimize_scalar[
    ...
    *,
    ...
    if method == "Brent" or method == "brent":
        if Bracket:
            return _brent_minimize[dtype, f](args, Bracket.value(), tol, maxiter)
        if bounds:
    ...
    if method == "Golden" or method == "golden":
        if Bracket:
            return _golden_minimize[dtype, f](args, Bracket.value(), tol, maxiter)
        if bounds:
            return _golden_minimize[dtype, f](args, bounds.value(), tol, maxiter)
