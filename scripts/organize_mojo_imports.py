...
Usage:
    python3 scripts/organize_mojo_imports.py scijo
...
def resolve_relative_module(module: str, file_path: Path, package_root: Path) -> str:
    ...
    `scijo.`-rooted form, based on where `file_path` sits under
    `package_root`. Non-relative modules are returned unchanged.

    SciJo convention: `__init__.mojo` re-exports use relative imports
    (`from .deriv import derivative`); every other file uses absolute
    imports (`from scijo.differentiate.utility import DiffResult`), never
    relative. Callers should only invoke this for non-`__init__.mojo` files
    — see `organize_file`.
    """
...
def organized_import_block(statements: list[ImportStatement]) -> str:
    ...
    # can share the same rendered title and must merge into one group
    ...
    # Cross-project convention (NuMojo, mojoBLAS, and now SciJo): each
    # import group is wrapped in a '# ===' / title / '# ===' banner.
...
def organize_file(path: Path, *, keep_unused: bool) -> str | None:
    ...
    # SciJo convention: internal imports are absolute (`scijo.x`), never
    # relative (`.x` / `..x`) — EXCEPT in `__init__.mojo` re-export files,
    # which use relative imports for their own submodules (`from .deriv
    # import derivative`) and are left untouched here. Resolve relative
    # modules elsewhere if we can determine the enclosing package root;
    # otherwise leave them as-is (e.g. this file isn't part of a package,
    # or is a top-level script).
    package_root = find_package_root(path)
    if package_root is not None and path.name != "__init__.mojo":
        statements = [
            ImportStatement(
                text=stmt.text,
                kind=stmt.kind,
